...
def render_dashboard_tab(df_all, stichtag, selected_cols, norm, deltas, compute_performance, handelstage):
    ...
    for col in selected_cols:
        ...
        c2.metric("YTD (%)", {perf_ytd}, f"{perf_ytd:.2f}%", delta_color=color_ytd)
        c3.metric("1 Woche (%)", {perf_week}, f"{perf_week:.2f}%", delta_color=color_week)
        c4.metric("1 Tag (%)", {perf_day}, f"{perf_day:.2f}%", delta_color=color_day)
